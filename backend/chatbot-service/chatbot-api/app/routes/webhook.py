...
async def product_webhook(payload: ProductWebhookPayload):
    ...
    ```
    """
    logger.info(f"Received webhook event: {payload.event} for product {payload.data.id}")
    
    try:
        if payload.event == "product.deleted":
            # Pour la suppression, on pourrait implémenter une vraie suppression
            # Pour l'instant on log juste
            logger.info(f"Product {payload.data.id} deleted (not removing from vector DB)")
            return {
                "status": "acknowledged",
                "event": payload.event,
