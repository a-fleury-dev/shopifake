...
def test_intent_product_search(client, mock_ollama_chat):
    ...
    mock_ollama_chat.return_value = '{"intent": "product_search"}'

    response = client.post("/intent", json={"prompt": "Show me cotton t-shirts"})
...
def test_intent_faq(client, mock_ollama_chat):
    ...
    mock_ollama_chat.return_value = '{"intent": "faq"}'

    response = client.post("/intent", json={"prompt": "What is your return policy?"})
...
def test_intent_other(client, mock_ollama_chat):
    ...
    mock_ollama_chat.return_value = '{"intent": "other"}'

    response = client.post("/intent", json={"prompt": "Hello, how are you?"})
...
def test_intent_fallback_regex_parsing(client, mock_ollama_chat):
    ...
    mock_ollama_chat.return_value = "The intent is product_search based on the query"

    response = client.post("/intent", json={"prompt": "Looking for shoes"})
...
def test_intent_fallback_to_other(client, mock_ollama_chat):
    ...
    mock_ollama_chat.return_value = "This is some random text without intent keywords"

    response = client.post("/intent", json={"prompt": "Random question"})
...
def test_intent_case_insensitive(client, mock_ollama_chat):
    ...
    response = client.post("/intent", json={"prompt": "Show me products"})
...
def test_intent_malformed_json(client, mock_ollama_chat):
    ...
    mock_ollama_chat.return_value = '{"intent": "product_search"'  # Missing brace

    response = client.post("/intent", json={"prompt": "Find shoes"})
...
def test_intent_empty_response(client, mock_ollama_chat):
    ...
    response = client.post("/intent", json={"prompt": "Test query"})
...
def test_intent_uses_correct_parameters(client, mock_ollama_chat):
    ...
    response = client.post("/intent", json={"prompt": "What's your shipping policy?"})

    assert response.status_code == 200

    # Verify correct parameters passed to our abstraction
