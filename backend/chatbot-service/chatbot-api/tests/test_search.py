...
def test_search_basic_query(client, mock_vectorstore):
    ...
    response = client.post("/search", json={"query": "cotton t-shirt", "top_k": 5})
...
def test_search_with_custom_top_k(client, mock_vectorstore):
    ...
    response = client.post("/search", json={"query": "sneakers", "top_k": 10})
...
def test_search_default_top_k(client, mock_vectorstore):
    ...
    response = client.post("/search", json={"query": "shoes"})
...
def test_search_no_results(client, mock_vectorstore):
    ...
    response = client.post("/search", json={"query": "nonexistent product xyz"})
...
def test_search_special_characters(client, mock_vectorstore):
    ...
    response = client.post(
        "/search", json={"query": "t-shirt & hoodie (100% cotton)", "top_k": 5}
    )
...
def test_search_with_high_scores(client, mock_vectorstore):
    ...
    mock_results = [
        SearchResult(
            id="prod-001", score=0.99, title="Product 1", snippet="Best match"
        ),
        SearchResult(
            id="prod-002", score=0.95, title="Product 2", snippet="Second best"
        ),
        SearchResult(
            id="prod-003", score=0.90, title="Product 3", snippet="Third best"
        ),
    ]
    mock_vectorstore["query_similar"].return_value = mock_results
