...
from app.models import (
    RecommendationRequest, RecommendationResponse, Product,
    IndexProductsRequest, IndexProductsResponse
)
from app.vectorstore import (
    recommend_by_product, recommend_by_text, get_product_by_id, upsert_products
)
...
async def get_recommendations(request: RecommendationRequest):
    ...
    if not request.product_id and not request.query:
        raise HTTPException(
            status_code=400,
            detail="Either 'product_id' or 'query' must be provided"
        )
    
    if request.product_id and request.query:
        raise HTTPException(
            status_code=400,
            detail="Provide either 'product_id' or 'query', not both"
        )
...
@router.get("/recommendations/product/{product_id}", response_model=RecommendationResponse)
...
async def index_products(request: IndexProductsRequest):
    ...
    if not request.items:
        raise HTTPException(
            status_code=400,
            detail="No products provided to index"
        )
