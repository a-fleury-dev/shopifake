...
# Script pour construire et déployer auth-service sur Kubernetes
# Usage: ./deploy.sh [tag]
...
echo -e "${GREEN}🚀 Déploiement de auth-service${NC}"
...
kubectl rollout status deployment/auth-service -n shopifake-prod --timeout=2m

echo ""
echo -e "${GREEN}🎉 auth-service déployé avec succès !${NC}"
echo ""
echo -e "${YELLOW}🔍 Commandes utiles:${NC}"
echo "  Pods:         kubectl get pods -n shopifake-prod"
echo "  Logs:         kubectl logs -n shopifake-prod -l app=auth-service -f"
...
echo "  HTTP:  http://auth.shopifake.duckdns.org"
echo "  HTTPS: https://auth.shopifake.duckdns.org"
echo "  Health: https://auth.shopifake.duckdns.org/actuator/health"
echo ""
